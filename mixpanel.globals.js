(function () {
  ...
  _.JSONEncode = (function () {
    return function (mixed_val) {
      ...
      var quote = function (string) {
        var escapable =
          /[\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g; // eslint-disable-line no-control-regex
        var meta = {
  ...
  var localStorageSupported = function (storage, forceCheck) {
    ...
    return (_localStorageSupported = _testStorageSupported(
  ...
  var sessionStorageSupported = function (storage, forceCheck) {
    ...
    return (_sessionStorageSupported = _testStorageSupported(
  ...
  var isRecordingExpired = function (serializedRecording) {
    ...
    return (
      !serializedRecording ||
      now > serializedRecording["maxExpires"] ||
      now > serializedRecording["idleExpires"]
    );
  };
  ...
  function getPropertiesFromElement(
    el,
    ev,
    blockAttrsSet,
    extraAttrs,
    ...
    if (
      shouldTrackElementDetails(el, ev, allowElementCallback, allowSelectors)
    ) {
  ...
  function getPropsForDOMEvent(ev, config) {
    ...
    if (
      ...
      _.each(
        targetElementList,
        function (el) {
          var shouldTrackDetails = shouldTrackElementDetails(
            el,
            ev,
            allowElementCallback,
            allowSelectors
      ...
      if (!explicitNoTrack) {
        ...
        if (target) {
          ...
          var targetProps = getPropertiesFromElement(
            target,
            ev,
            blockAttrsSet,
            captureExtraAttrs,
   ...
   */
  function shouldTrackElementDetails(
    el,
    ev,
    allowElementCallback,
    allowSelectors
  ...
  RageClickTracker.prototype.isRageClick = function (event, options) {
    options = options || {};
    var thresholdPx =
      options["threshold_px"] || DEFAULT_RAGE_CLICK_THRESHOLD_PX;
    var timeoutMs = options["timeout_ms"] || DEFAULT_RAGE_CLICK_TIMEOUT_MS;
    var clickCount = options["click_count"] || DEFAULT_RAGE_CLICK_CLICK_COUNT;
    var interactiveElementsOnly =
      options["interactive_elements_only"] ||
      DEFAULT_RAGE_CLICK_INTERACTIVE_ELEMENTS_ONLY;
    ...
    if (
      ...
      Math.sqrt(Math.pow(x - lastClick.x, 2) + Math.pow(y - lastClick.y, 2)) <
        thresholdPx
    ) {
  ...
  DeadClickTracker.prototype.addClick = function (event) {
    var element =
      this.shadowDOMObserver && this.shadowDOMObserver.getEventTarget(event);
  ...
  DeadClickTracker.prototype.startTracking = function () {
    ...
    if (window.customElements) {
      try {
        this.shadowDOMObserver = new ShadowDOMObserver(function () {
          self.recordChangeEvent();
        }, MUTATION_OBSERVER_CONFIG);
  ...
  var PAGEVIEW_OPTION_URL_WITH_PATH_AND_QUERY_STRING =
  ...
  Autocapture.prototype.currentUrlBlocked = function () {
    ...
    if (allowUrlRegexes.length) {
      ...
      for (i = 0; i < allowUrlRegexes.length; i++) {
        ...
        try {
          ...
          logger$4.critical(
            "Error while checking block URL regex: " + allowRegex,
            err
          );
          return true;
        }
    ...
    for (i = 0; i < blockUrlRegexes.length; i++) {
      try {
        ...
        logger$4.critical(
          "Error while checking block URL regex: " + blockUrlRegexes[i],
          err
        );
        return true;
      }
  ...
  Autocapture.prototype.trackDomEvent = function (ev, mpEventName) {
    ...
    var isCapturedForHeatMap =
      this.mp.is_recording_heatmap_data() &&
      ((mpEventName === MP_EV_CLICK && !this.getConfig(CONFIG_TRACK_CLICK)) ||
        (mpEventName === MP_EV_RAGE_CLICK &&
          !this._getClickTrackingConfig(CONFIG_TRACK_RAGE_CLICK)) ||
        (mpEventName === MP_EV_DEAD_CLICK &&
          !this._getClickTrackingConfig(CONFIG_TRACK_DEAD_CLICK)));
  ...
  Autocapture.prototype.initPageListeners = function () {
    ...
    if (
      ...
      !this.getConfig(CONFIG_TRACK_PAGE_LEAVE) &&
  ...
  Autocapture.prototype._trackPageLeave = function (
    ...
    var viewportHeight = Math.max(
      document$1.documentElement.clientHeight,
      win.innerHeight || 0
    );
    var scrollPercentage = Math.round(
      (Math.max(this.maxScrollViewDepth - viewportHeight, 0) /
        (currentScrollHeight - viewportHeight)) *
        100
    );
    var foldLinePercentage = Math.round(
      (viewportHeight / currentScrollHeight) * 100
    );
    ...
    var props = _.extend(
      {
        ...
        $viewportWidth: Math.max(
          document$1.documentElement.clientWidth,
          win.innerWidth || 0
        ),
  ...
  Autocapture.prototype.initClickTracking = function () {
    ...
    if (
      !this.getConfig(CONFIG_TRACK_CLICK) &&
      !this.mp.get_config("record_heatmap_data")
    ) {
      return;
    }
  ...
  Autocapture.prototype.initDeadClickTracking = function () {
    ...
    if (!this._deadClickTracker) {
      this._deadClickTracker = new DeadClickTracker(
        function (deadClickEvent) {
    ...
    if (!this.listenerDeadClick) {
      this.listenerDeadClick = function (ev) {
        var currentDeadClickConfig = this._getClickTrackingConfig(
  ...
  Autocapture.prototype.initPageviewTracking = function () {
    ...
    this.listenerLocationchange = safewrap(
      function () {
        if (this.currentUrlBlocked()) {
          return;
        }
        ...
        var didPathChange =
          ...
          previousTrackedUrl.split("#")[0].split("?")[0];
        var trackPageviewOption = this.pageviewTrackingConfig();
        if (trackPageviewOption === PAGEVIEW_OPTION_FULL_URL) {
          shouldTrack = currentUrl !== previousTrackedUrl;
        } else if (
          trackPageviewOption === PAGEVIEW_OPTION_URL_WITH_PATH_AND_QUERY_STRING
        ) {
          shouldTrack =
            currentUrl.split("#")[0] !== previousTrackedUrl.split("#")[0];
  ...
  Autocapture.prototype.initRageClickTracking = function () {
    ...
    this.listenerRageClick = function (ev) {
      var currentRageClickConfig = this._getClickTrackingConfig(
        CONFIG_TRACK_RAGE_CLICK
      );
  ...
  Autocapture.prototype.initScrollTracking = function () {
    ...
    var scrollTrackFunction = function () {
      ...
      try {
        ...
        var scrollPercentage = Math.round(
          (scrollTop / (scrollHeight - win.innerHeight)) * 100
        );
  ...
  Autocapture.prototype.initPageLeaveTracking = function () {
    ...
    document$1.removeEventListener(
      EV_VISIBILITYCHANGE,
      this.listenerPageLeaveVisibilitychange
    );
    win.removeEventListener(
      EV_MP_LOCATION_CHANGE,
      this.listenerPageLeaveLocationchange
    );
    ...
    this.listenerPageLeaveLocationchange = safewrap(
      function (ev) {
        if (this.currentUrlBlocked()) {
          return;
        }
  ...
  FeatureFlagManager.prototype.updateContext = function (newContext, options) {
    ...
    var oldContext =
      options && options["replace"] ? {} : this.getConfig(CONFIG_CONTEXT);
  ...
  FeatureFlagManager.prototype.getVariantSync = function (
    featureName,
    fallback
  ) {
  ...
  FeatureFlagManager.prototype.getVariantValue = function (
    featureName,
    fallbackValue
  ) {
    return this.getVariant(featureName, { value: fallbackValue })
  ...
  FeatureFlagManager.prototype.getFeatureData = function (
    featureName,
    fallbackValue
  ) {
    logger$3.critical(
  ...
  FeatureFlagManager.prototype.getVariantValueSync = function (
    featureName,
    fallbackValue
  ) {
  ...
  FeatureFlagManager.prototype.isEnabled = function (
    ...
    return this.getVariantValue(featureName)
      .then(
        function () {
  ...
  FeatureFlagManager.prototype.isEnabledSync = function (
    featureName,
    fallbackValue
  ) {
  ...
  FeatureFlagManager.prototype.trackFeatureCheck = function (
    featureName,
    feature
  ) {
    ...
    if (feature["is_experiment_active"] !== "undefined") {
      trackingProperties["$is_experiment_active"] =
        feature["is_experiment_active"];
    }
  ...
  FeatureFlagManager.prototype["are_flags_ready"] =
    FeatureFlagManager.prototype.areFlagsReady;
  FeatureFlagManager.prototype["get_variant"] =
    FeatureFlagManager.prototype.getVariant;
  FeatureFlagManager.prototype["get_variant_sync"] =
    FeatureFlagManager.prototype.getVariantSync;
  FeatureFlagManager.prototype["get_variant_value"] =
    FeatureFlagManager.prototype.getVariantValue;
  FeatureFlagManager.prototype["get_variant_value_sync"] =
    FeatureFlagManager.prototype.getVariantValueSync;
  FeatureFlagManager.prototype["is_enabled"] =
    FeatureFlagManager.prototype.isEnabled;
  FeatureFlagManager.prototype["is_enabled_sync"] =
    FeatureFlagManager.prototype.isEnabledSync;
  FeatureFlagManager.prototype["update_context"] =
    FeatureFlagManager.prototype.updateContext;

  // Deprecated method
  FeatureFlagManager.prototype["get_feature_data"] =
    FeatureFlagManager.prototype.getFeatureData;
          ...
          if (valY && valY !== i) {
            ...
            if (storage.getItem(keyY) === i) {
              ...
              if (!localStorageSupported(storage, true)) {
                reject(
                  new Error("localStorage support dropped while acquiring lock")
                );
              }
              return false;
            }
  ...
  var RequestQueue = function (storageKey, options) {
    ...
    this.reportError =
      options.errorReporter || _.bind(logger$1.error, logger$1);
    ...
    if (options.enqueueThrottleMs) {
      this.enqueuePersisted = batchedThrottle(
        _.bind(this._enqueuePersisted, this),
        options.enqueueThrottleMs
      );
    } else {
  ...
  RequestQueue.prototype._enqueuePersisted = function (queueEntries) {
    ...
    return this.lock.withLock(enqueueItem, this.pid).catch(
      _.bind(function (err) {
          ...
          _.bind(function (storedQueue) {
            if (storedQueue.length) {
              ...
              for (var i = 0; i < storedQueue.length; i++) {
                ...
                if (
                  new Date().getTime() > item["flushAfter"] &&
                  !idsInBatch[item["id"]]
                ) {
                  item.orphaned = true;
                  batch.push(item);
  ...
  RequestQueue.prototype.removeItemsByID = function (ids) {
    ...
    if (!this.usePersistence) {
      ...
      return this.lock.withLock(removeFromStorage, this.pid).catch(
        _.bind(function (err) {
          this.reportError("Error acquiring storage lock", err);
  ...
  RequestBatcher.prototype.flush = function (options) {
    ...
    return this.queue
      .fillBatch(currentBatchSize)
      .then(
        _.bind(function (batch) {
                    ...
                    _.bind(function (item) {
                      ...
                      if (itemId) {
                        this.itemIdsSentSuccessfully[itemId] =
                          this.itemIdsSentSuccessfully[itemId] || 0;
                        this.itemIdsSentSuccessfully[itemId]++;
          ...
          return this.sendRequestPromise(dataForRequest, requestOptions).then(
            batchSendCallback
          );
        }, this)
      )
  ...
  function _getStorage(options) {
    options = options || {};
    return options.persistenceType === "localStorage"
      ? _.localStorage
      : _.cookie;
  }
  ...
  function _getStorageKey(token, options) {
    options = options || {};
    return (
      (options.persistencePrefix || GDPR_DEFAULT_PERSISTENCE_PREFIX) + token
    );
  }
  ...
  function _addOptOutCheck(method, getConfigValue) {
    return function () {
      ...
      try {
        ...
        var persistenceType = getConfigValue.call(
          this,
          "opt_out_tracking_persistence_type"
        );
        var persistencePrefix = getConfigValue.call(
          this,
        ...
        console.error(
          "Unexpected error when checking tracking opt-out status: " + err
        );
      }
  ...
  MixpanelGroup.prototype._init = function (
    mixpanel_instance,
    group_key,
    group_id
  ) {
  ...
  MixpanelPeople.prototype._send_request = function (data, callback) {
    ...
    var had_persisted_distinct_id = this._mixpanel.get_property(
  ...
  MixpanelPeople.prototype._flush_one_queue = function (
    action,
    action_method,
    callback,
    queue_to_params_fn
    ...
    var _this = this;
    var queued_data = _.extend(
      {},
      this._mixpanel["persistence"].load_queue(action)
    );
    var action_params = queued_data;

    if (
      !_.isUndefined(queued_data) &&
      _.isObject(queued_data) &&
      !_.isEmptyObject(queued_data)
    ) {
  ...
  MixpanelPeople.prototype._flush = function (
    ...
    if (
      !_.isUndefined($remove_queue) &&
      _.isArray($remove_queue) &&
      $remove_queue.length
    ) {
      var $remove_item;
  ...
  MixpanelPersistence.prototype.register_once = function (
    ...
    if (_.isObject(props)) {
      ...
      this.expire_days =
        typeof days === "undefined" ? this.default_expiry : days;
  ...
  MixpanelPersistence.prototype.set_cross_subdomain = function (
    cross_subdomain
  ) {
  ...
  IDBStorageWrapper.prototype.makeTransaction = function (mode, storeCb) {
    ...
    return this.dbPromise.then(doTransaction).catch(
      function (err) {
  ...
  var create_mplib = function (token, config, name) {
    ...
    win.dispatchEvent(
      new win.CustomEvent("$mp_sdk_to_extension_event", {
        detail: {
          instance: instance,
          source: source,
          token: token,
  ...
  MixpanelLib.prototype._init = function (token, config, name) {
    ...
    this.set_config(
      _.extend({}, DEFAULT_CONFIG, variable_features, config, {
        name: name,
        token: token,
        callback_fn:
          (name === PRIMARY_INSTANCE_NAME
    ...
    this["persistence"] = this["cookie"] = new MixpanelPersistence(
      this["config"]
    );
    ...
    if (mode === SETTING_STRICT || mode === SETTING_FALLBACK) {
      this._fetch_remote_settings(mode).then(
        _.bind(function () {
  ...
  MixpanelLib.prototype._init_tab_id = function () {
    if (this.get_config("disable_persistence")) {
      ...
      try {
        var key_suffix =
          this.get_config("name") + "_" + this.get_config("token");
        ...
        if (
          _.sessionStorage.get(should_generate_new_tab_id_key) ||
          !_.sessionStorage.get(tab_id_key)
        ) {
  ...
  MixpanelLib.prototype._should_load_recorder = function () {
    ...
    var recording_registry_idb = new IDBStorageWrapper(
      RECORDING_REGISTRY_STORE_NAME
    );
    var tab_id = this.get_tab_id();
    return recording_registry_idb
      .init()
      .then(function () {
      ...
      .then(function (recordings) {
        for (var i = 0; i < recordings.length; i++) {
          ...
          if (
            isRecordingExpired(recordings[i]) ||
            recordings[i]["tabId"] === tab_id
          ) {
            return true;
          }
        }
  ...
  MixpanelLib.prototype._check_and_start_session_recording =
    addOptOutCheckMixpanelLib(function (force_start) {
  ...
  MixpanelLib.prototype.is_recording_heatmap_data = function () {
    return (
      this._get_session_replay_id() && this.get_config("record_heatmap_data")
    );
  };
  ...
  MixpanelLib.prototype.get_session_replay_url = function () {
    ...
    if (replay_id) {
      ...
      replay_url =
        "https://mixpanel.com/projects/replay-redirect?" + query_params;
    }
    return replay_url;
  };
  ...
  MixpanelLib.prototype._send_request = function (
    ...
    var use_sendBeacon =
      sendBeacon &&
      use_post &&
      options.transport.toLowerCase() === "sendbeacon";
  ...
  MixpanelLib.prototype._fetch_remote_settings = function (mode) {
    ...
    if (!win["AbortController"]) {
      console.critical(
        "Remote settings unavailable: missing minimum required APIs"
      );
      disableRecordingIfStrict();
      return Promise.resolve();
    }

    var settings_endpoint =
      this.get_api_host("settings") +
      "/" +
      this.get_config("api_routes")["settings"];
    var request_params = {
      $lib_version: Config.LIB_VERSION,
      mp_lib: "web",
      sdk_config: "1"
    ...
    return win["fetch"](full_url, fetchOptions)
      ...
      var batcher_for = _.bind(function (attrs) {
        return new RequestBatcher(attrs.queue_key, {
          ...
          sendRequestFunc: _.bind(function (data, options, cb) {
            ...
            this._send_request(
              this.get_api_host(attrs.api_name) +
                "/" +
                api_routes[attrs.api_name],
  ...
  MixpanelLib.prototype._track_or_batch = function (options, callback) {
    ...
    var send_request_immediately = _.bind(function () {
      if (!send_request_options.skip_hooks) {
        truncated_data = this._run_hook(
          "before_send_" + options.type,
          truncated_data
        );
        if (truncated_data) {
  ...
  MixpanelLib.prototype.track = addOptOutCheckMixpanelLib(
    function (event_name, properties, options, callback) {
      ...
      if (!_.isUndefined(start_timestamp)) {
        ...
        properties["$duration"] = parseFloat(
          (duration_in_ms / 1000).toFixed(3)
        );
      }
  ...
  MixpanelLib.prototype._remove_group_from_cache = function (
    group_key,
    group_id
  ) {
  ...
  MixpanelLib.prototype.get_group = function (group_key, group_id) {
    ...
    if (
      group === undefined ||
      group._group_key !== group_key ||
      group._group_id !== group_id
    ) {
   ...
   * @param {boolean} [days_or_options.persistent=true] - whether to put in persistent storage (cookie/localStorage)
   */
  MixpanelLib.prototype.register_once = function (
    props,
    default_value,
    days_or_options
  ) {
    var ret = this._run_hook(
    ...
    if (options["persistent"]) {
      ...
      _.each(
        ...
        function (val, prop) {
          if (
            !this.unpersisted_superprops.hasOwnProperty(prop) ||
            this.unpersisted_superprops[prop] === default_value
          ) {
  ...
  MixpanelLib.prototype.identify = function (
    ...
    if (new_distinct_id && previous_distinct_id !== new_distinct_id) {
      ...
      if (
        typeof new_distinct_id === "string" &&
        new_distinct_id.indexOf(DEVICE_ID_PREFIX) === 0
      ) {
    ...
    if (
      ...
      new_distinct_id !== this.get_property(ALIAS_ID_KEY)
    ) {
  ...
  MixpanelLib.prototype.set_config = function (config) {
    if (_.isObject(config)) {
      ...
      if (
        ("autocapture" in config || "record_heatmap_data" in config) &&
        this.autocapture
      ) {
        this.autocapture.init();
      }
            ...
            if (_.isFunction(hook_value)) {
              ...
              for (var i = 0; i < hook_value.length; i++) {
                if (!_.isFunction(hook_value[i])) {
                  console.critical(
                    "Invalid hook added. Hook is not a function"
                  );
                }
  ...
  MixpanelLib.prototype.get_api_host = function (endpoint_type) {
    return (
      this.get_config("api_hosts")[endpoint_type] || this.get_config("api_host")
    );
  };
  ...
  MixpanelLib.prototype._gdpr_update_persistence = function (options) {
    ...
    if (
      ...
      this["persistence"].disabled !== disabled
    ) {
  ...
  MixpanelLib.prototype["stop_session_recording"] =
    MixpanelLib.prototype.stop_session_recording;
  MixpanelLib.prototype["pause_session_recording"] =
    MixpanelLib.prototype.pause_session_recording;
  MixpanelLib.prototype["resume_session_recording"] =
    MixpanelLib.prototype.resume_session_recording;
  MixpanelLib.prototype["get_session_recording_properties"] =
    MixpanelLib.prototype.get_session_recording_properties;
  MixpanelLib.prototype["get_session_replay_url"] =
    MixpanelLib.prototype.get_session_replay_url;
  MixpanelLib.prototype["get_tab_id"] = MixpanelLib.prototype.get_tab_id;
  MixpanelLib.prototype["DEFAULT_API_ROUTES"] = DEFAULT_API_ROUTES;

  // Exports intended only for testing
  MixpanelLib.prototype["__get_recorder"] =
    MixpanelLib.prototype.__get_recorder;
  ...
  function init_from_snippet(bundle_loader) {
    ...
    if (
      mixpanel_master["__loaded"] ||
      (mixpanel_master["config"] && mixpanel_master["persistence"])
    ) {
